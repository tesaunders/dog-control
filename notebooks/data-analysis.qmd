...
#| fig-cap: "A drop in the recorded dog population in 2018 was due to the removal of duplicate entries in the Auckland Council dog database, and in 2025 was due to the updating of the database during a registration campaign."
...
impound_rate <-
  ...
  select(-category) |> 
  mutate(
    impound_rate = (impounds / dogs) * 1000
  )
...
ggplot(impound_result, aes(x = year, y = count, fill = category)) +
  ...
  labs(title = "Outcome For Impounded Dogs Over Time",
       subtitle = "Euthanasia increases since COVID lockdowns")
```
...
p1 <- breed_data |>
  ...
  labs(title = "Breed Profile: Auckland Dog Population vs Euthanised",
       subtitle = "Red 'differences' indicate higher euthanisation than expected", x = NULL, y = NULL) +
  theme_minimal() +
  theme(
    ...
    axis.ticks.length.x = unit(3, "pt") # Controls how "small" the ticks are
  )
...
metric_labels <- c(
  "animal_desexed" = "Desexed",
  "classification" = "Classification",
  "current_reg" = "Registered",
  "owner_reg" = "RDOL"
)
...
ggplot(plot_data, aes(x = p_board, y = fct_reorder(local_board, p_board), fill = metric)) +
  ...
  scale_fill_manual(values = region_colours[1:4]) +
  plot_theme() +
...
corrplot(cor_results$r, 
         method = "color", 
         type = "upper", 
         addCoef.col = "black", # Show the actual correlation number
         tl.col = "black", 
         diag = FALSE,
         sig.level = 0.05, 
         insig = "blank")
